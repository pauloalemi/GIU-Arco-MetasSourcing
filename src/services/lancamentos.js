...
export async function initUserSheet(token, userName) {
  ...
  if (rows.length === 0) {
    await appendRows(token, userName, [['data', 'projeto_id', 'projeto_nome', 'triagem']])
  }
}
...
export async function upsertLancamento(token, userName, data, projectId, projectName, triagem) {
  const rows = await readSheet(token, userName).catch(() => [])
  ...
  if (existingIndex >= 0) {
    ...
    await updateRange(token, `${userName}!A${rowIndex}:D${rowIndex}`, [
      [data, projectId, projectName, triagem],
    ])
  } else {
    await appendRows(token, userName, [[data, projectId, projectName, triagem]])
  }
}
